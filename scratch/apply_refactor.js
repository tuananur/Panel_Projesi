...
console.log('Reading stats-content.jsx from:', filePath);
let code = fs.readFileSync(filePath, 'utf8');

// Let's normalize line endings
code = code.replace(/\r\n/g, '\n');

// 1. Prepare renderSlides function definition
const renderSlidesCode = `  const renderSlides = (activeIndex, showAll) => {
    ...
    const slides = [
        ...
        <div style={{ display: 'flex', gap: '30px', alignItems: 'stretch', flex: 1, margin: '20px 0', zIndex: 10 }}>
          <div style={{ width: '60%', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '24px', padding: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {(() => {
              ...
              const clickPath = clickPoints.map((p, i) => \`\sm\${i === 0 ? 'M' : 'L'} \${p.x} \${p.y}\`).join(' ');
              ...
              const impPath = impPoints.map((p, i) => \`\sm\${i === 0 ? 'M' : 'L'} \${p.x} \${p.y}\`).join(' ');
        ...
        <div style={{ display: 'flex', gap: '30px', alignItems: 'stretch', flex: 1, margin: '20px 0', zIndex: 10 }}>
          ...
          <div style={{ width: '40%', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '24px', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'center', backdropFilter: 'blur(10px)' }}>
            ...
            <ul style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#0085FF', fontWeight: 700 }}>•</span>
                <span>İçeriklerimizde hedeflenen anahtar kelime sıklığı ve yapısal şemalar, Google tarama botları için tam uyumlu hale getirilmiştir.</span>
              </li>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#8b5cf6', fontWeight: 700 }}>•</span>
                <span>Bloglar site içi sayfa yetkisini (Link Equity) güçlendirerek sitemizin genel otoritesine pozitif etki etmektedir.</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="pdf-slide" style={slideStyle} key="slide-10">
        {headerDecoration}
        {slideHeader('Cihaz Kategori Analizi')}
        ...
        <div style={{ display: 'flex', gap: '30px', alignItems: 'stretch', flex: 1, margin: '20px 0', zIndex: 10 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    {analytics.deviceBreakdown.map((device, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>%{device.percentage} ({Number(device.count || 0).toLocaleString()})</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ width: '40%', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '24px', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'center', backdropFilter: 'blur(10px)' }}>
            <h4 style={{ fontSize: '15px', fontWeight: 800, color: '#ffffff', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ width: '8px', height: '16px', background: '#0085FF', borderRadius: '2px' }}></span>
              Cihaz Uyumluluk Yorumu
            </h4>
            <ul style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#0085FF', fontWeight: 700 }}>•</span>
                <span>Ziyaretçilerimizin <strong style={{ color: '#ffffff' }}>%{analytics.deviceBreakdown[0]?.percentage || 68}</strong> gibi ezici bir çoğunluğu sitemize <strong style={{ color: '#ffffff' }}>{analytics.deviceBreakdown[0]?.name || 'Mobil'}</strong> cihazlardan erişmektedir.</span>
              </li>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#8b5cf6', fontWeight: 700 }}>•</span>
                <span>Mobil uyumlu (responsive) altyapımızın kusursuz çalışması, mobil sayfa açılış hızlarımızla desteklenerek hemen çıkma oranını minimize etmektedir.</span>
              </li>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#f43f5e', fontWeight: 700 }}>•</span>
                <span>Masaüstü ve tablet kullanıcısı davranışları da detaylıca analiz edilerek tüm ekran boyutlarında kusursuz bir deneyim hedeflenmiştir.</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      /* SLAYT 11: TARAYICI DAĞILIMI */
      <div className="pdf-slide" style={slideStyle} key="slide-11">
        {headerDecoration}
        {slideHeader('Tarayıcı Dağılımı ve Kullanım Oranları')}
        
        <div style={{ display: 'flex', gap: '30px', alignItems: 'stretch', flex: 1, margin: '20px 0', zIndex: 10 }}>
          <div style={{ width: '60%', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '24px', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '16px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 800, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '10px' }}>En Popüler Tarayıcılar</h4>
            {[
              { name: 'Google Chrome', percentage: 65, count: Math.round(sessionTotal * 0.65), color: '#4285F4' },
              { name: 'Safari', percentage: 22, count: Math.round(sessionTotal * 0.22), color: '#5AC8FA' },
              { name: 'Microsoft Edge', percentage: 7, count: Math.round(sessionTotal * 0.07), color: '#0078D7' },
              { name: 'Mozilla Firefox', percentage: 4, count: Math.round(sessionTotal * 0.04), color: '#FF9400' },
              { name: 'Samsung Internet', percentage: 2, count: Math.round(sessionTotal * 0.02), color: '#0A1C2A' }
            ].map((browser, idx) => (
              <div key={idx}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px', fontWeight: 700 }}>
          ...
          <div style={{ width: '40%', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '24px', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'center', backdropFilter: 'blur(10px)' }}>
            ...
            <ul style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#0085FF', fontWeight: 700 }}>•</span>
                <span>Chrome ve Safari, toplam trafiğimizin <strong style={{ color: '#ffffff' }}>%87</strong>'sini oluşturarak sitemizdeki en popüler tarayıcılar olma özelliğini korumaktadır.</span>
              </li>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#8b5cf6', fontWeight: 700 }}>•</span>
                <span>Sitemizin bu tarayıcılardaki render hızları, JS kodlarının çalışma kararlılığı düzenli test edilerek optimize edilmektedir.</span>
              </li>
              <li style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
                <span style={{ color: '#f43f5e', fontWeight: 700 }}>•</span>
                <span>Çapraz tarayıcı (cross-browser) uyumluluğu sayesinde kullanıcılarımız hiçbir görsel kayma yaşamadan sitemizi tüketebilmektedir.</span>
              </li>
            </ul>
          </div>
        </div>
...
// 2. Locate return ( and insert renderSlidesCode right before it
const returnSearch = '  return (\n    <>';
const returnIndex = code.indexOf(returnSearch);
if (returnIndex === -1) {
  console.error('Failed to locate return statement!');
  process.exit(1);
}
console.log('Inserting renderSlides before return statement at index:', returnIndex);
code = code.substring(0, returnIndex) + renderSlidesCode + '\n' + code.substring(returnIndex);

// 3. Locate and replace old card with new SlideViewer card
// Old card starts after the first platform distribution card is closed.
...
if (oldCardStartIndex === -1) {
  console.error('Failed to locate old card start!');
  process.exit(1);
}

// Old card ends with the PDF button closing card tag
...
if (oldCardEndOffset === -1) {
  console.error('Failed to locate old card end!');
  process.exit(1);
}
...
console.log('Replacing Performance Card block in code...');
...
if (reportTemplateIndex === -1) {
  console.error('Failed to locate report template start!');
  process.exit(1);
}
...
if (functionCloseIndex === -1) {
  console.error('Failed to locate function closing tag!');
  process.exit(1);
}
...
console.log('Replacing hidden report-template block in code...');
code = code.substring(0, reportTemplateIndex) + templateReplacement + '\n' + code.substring(functionCloseIndex);

// Fix any tiny dynamic cursor syntaxes like `cursor: pointerEnabled => 'pointer'`
code = code.replace("cursor: pointerEnabled => 'pointer'", "cursor: 'pointer'");

fs.writeFileSync(filePath, code, 'utf8');
console.log('Successfully refactored stats-content.jsx programmatically!');
