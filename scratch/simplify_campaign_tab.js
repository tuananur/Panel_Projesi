...
const stateHookPos = content.indexOf('const [createFormData, setCreateFormData] = useState(');
const newStateCode = `
...
`;
content = content.slice(0, stateHookPos) + newStateCode + content.slice(stateHookPos);

// 2. Add Modal UI right before the return statement's main div
const returnPos = content.lastIndexOf('return ('); // FIX: Use lastIndexOf to find the main return
const modalCode = `
...
`;
// Insert right after the main return (
const firstDivPos = content.indexOf('<div', returnPos);
content = content.slice(0, firstDivPos) + modalCode + content.slice(firstDivPos);


// 3. Replace Kampanya Tab Content
const campaignTabStart = content.indexOf("{activeTab === 'campaigns' && (");
const campaignTabEnd = content.indexOf("{activeTab === 'adsets' && (");
...
content = content.slice(0, campaignTabStart) + newCampaignTab + content.slice(campaignTabEnd);

fs.writeFileSync(targetPath, content);
console.log('Successfully added Objective Modal and simplified Campaign tab.');
