...
const srcPath = 'c:\\Users\\Casper\\Desktop\\TEST\\src\\app\\dashboard\\client\\[id]\\meta\\meta-content.jsx';
const dstPath = 'c:\\Users\\Casper\\Desktop\\TEST\\src\\app\\dashboard\\client\\[id]\\meta\\create\\create-meta-client.jsx';
...
const extractBlock = (startRegex, endRegex) => {
    ...
    for(let i=0; i<lines.length; i++) {
        if(startIndex === -1 && startRegex.test(lines[i])) startIndex = i;
...
const ctaLabels = extractBlock(/const CTA_LABELS/, /};/);
const renderAdPreview = extractBlock(/const renderAdPreview =/, /  };\n\n  const renderAdFormFields/);
const renderAdFormFields = extractBlock(/const renderAdFormFields =/, /  };\n\n  const renderAudienceCard/);
const renderAudienceCard = extractBlock(/const renderAudienceCard =/, /  };\n\n  return \(/);

const renderAdPreviewClean = renderAdPreview.split('const renderAdFormFields')[0].trim();
const renderAdFormFieldsClean = renderAdFormFields.split('const renderAudienceCard')[0].trim();
const renderAudienceCardClean = renderAudienceCard.split('return (')[0].trim();
...
export default function CreateMetaClient({ clientId, initialCampaigns, initialAdSets }) {
  ...
  __RENDER_AD_PREVIEW__

  __RENDER_AD_FORM_FIELDS__

  __RENDER_AUDIENCE_CARD__
...
const newFileContent = template
    ...
    .replace('__RENDER_AD_PREVIEW__', renderAdPreviewClean)
    .replace('__RENDER_AD_FORM_FIELDS__', renderAdFormFieldsClean)
    .replace('__RENDER_AUDIENCE_CARD__', renderAudienceCardClean);

fs.writeFileSync(dstPath, newFileContent, 'utf8');
console.log('Successfully wrote create-meta-client.jsx');
