...
// Let's extract the body of renderSlides from line 481 to line 1263
...
// Standard JS and React globals to ignore
...
// Scan through the words and see which ones are not defined in the function body
words.forEach(word => {
  if (globals.has(word)) return;
  // If the word is defined with const/let/var/function in the body, it's local
  ...
  outerScopeVars.push(word);
});

console.log('Variables used in renderSlides that are from the outer scope:');
console.log(outerScopeVars);

// Check if these variables are defined in StatsContent or imported
const statsContentCode = lines.slice(101).join('\n');
outerScopeVars.forEach(varName => {
  // Let's check if the variable is defined or in scope
  const isImported = code.includes('import') && code.includes(varName);
  const isDefined = new RegExp('\\b(const|let|var|function|class)\\s+' + varName + '\\b').test(code);
  const isProp = varName === 'client' || varName === 'metaResult' || varName === 'googleResult' || varName === 'analyticsResult';
  
  if (!isImported && !isDefined && !isProp) {
    console.log(`⚠️ WARNING: "${varName}" might not be defined or imported!`);
  } else {
    console.log(`✓ "${varName}" is in scope (${isProp ? 'Prop' : isImported ? 'Imported' : 'Local/Defined'})`);
  }
});
