...
import { can, getRolePermissions, saveRolePermissions } from '@/lib/permissions';
...
export async function createWorkItemAction(formData) {
  const session = await getSession();
  if (!(await canAccessWorkItems(session)) || !isWorkManagerRole(session.role)) {
    return { error: '�� atama yetkiniz yok.' };
  }
  ...
  try {
    ...
    await logActivity('CREATE', 'WORK_ITEM', `${title} i�i ${item.assignee.username} kullan�c�s�na atand�.`, clientId);
